...
import { Bug, BugPayload, BugPriority, BugStatus } from '../models/bug.model';
import { environment } from '../../../environments/environment';

/**
 * Unit-Test des Service-Layers (Bewertungspunkt "Unit Test Service").
 *
 * Der HTTP-Verkehr wird mit dem `HttpTestingController` abgefangen. Geprüft
 * wird, dass jede CRUD-Methode die richtige URL mit der richtigen HTTP-Methode
 * aufruft und dass die JSON-Antwort korrekt auf die Klasse `Bug` abgebildet wird.
 */
describe('BugService', () => {
  const baseUrl = `${environment.apiUrl}/bugs`;
  ...
  it('getAll() ruft GET /api/bugs auf und bildet die Antwort auf Bug[] ab', () => {
    let result: Bug[] | undefined;
    service.getAll().subscribe((bugs) => (result = bugs));
  ...
  it('getById() ruft GET /api/bugs/{id} auf', () => {
    let result: Bug | undefined;
  ...
  it('remove() ruft DELETE /api/bugs/{id} auf', () => {
  ...
  it('meldet Fehler des Backends an den Aufrufer weiter', () => {
    ...
    service.getById(99).subscribe({
      next: () => fail('Es wurde ein Fehler erwartet.'),
